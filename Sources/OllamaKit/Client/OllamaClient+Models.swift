...
extension OllamaClient {
    ...
    /// Requests the Ollama API to pull a specific model, returning the result as a Combine publisher.
    ///
    /// This method provides a reactive approach to request a model pull operation. It accepts ``PullModelRequest`` and returns a Combine publisher that completes when the deletion operation is finished.
    ///
    /// ```swift
    /// let OllamaClient = OllamaClient(baseURL: URL(string: "http://localhost:11434")!)
    /// let requestData = PullModelRequest(/* parameters */)
    ///
    /// OllamaClient.pullModel(data: requestData)
    ///     .sink(receiveCompletion: { completion in
    ///         // Handle completion
    ///     }, receiveValue: {
    ///         // Handle successful completion of the pull operation
    ///     })
    ///     .store(in: &cancellables)
    /// ```
    ///
    /// - Parameter data: The ``PullModelRequest`` used to request the model deletion.
    /// - Returns: An `AnyPublisher<PullModelResponse, AFError>` emitting the live stream of chat responses from the Ollama API.
    ...
    /// Asynchronously requests the Ollama API to fetch all library models.
    ///
    /// This method sends a request to the Ollama API to fetch all library models.
    ///
    /// ```swift
    /// let OllamaClient = OllamaClient(baseURL: URL(string: "http://localhost:11434")!)
    /// try await OllamaClient.libraryModels()
    /// ```
    ///
    /// - Returns : An list contains all available library models.
    /// - Throws: An error if the request to delete the model fails.
...
//    }
    ...
    public func modelTags(_ name: String) async throws -> [ModelInfo] {
        let html = await withCheckedContinuation { continuation in
            AF.request(router.modelTags(data: name))
                .responseString { response in
                    continuation.resume(returning: response.value)
                }
        }
        guard let html else { return [] }
        return try SwiftSoup.parse(html)
            .getElementsByClass("group")
            .compactMap{ try $0.select("a").attr("href") }
            .compactMap{ $0.split(separator: "/").last }
            .compactMap{ ModelInfo(name: String($0)) }
    }
}
